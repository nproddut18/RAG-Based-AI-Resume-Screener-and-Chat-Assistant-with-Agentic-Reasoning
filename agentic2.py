    ...
    try:
        ...
        if r.status_code != 200:
            try:
                ...
                err = {"raw_text": r.text}
            return f"[Error] API request failed ({r.status_code}): {json.dumps(err)[:1200]}"
...
def tool_answer_question(context_chunks: List[RetrievedChunk], question: str) -> Dict[str, Any]:
    ...
    prompt = (
        f"Use ONLY the context to answer. If not supported, say 'I don't know'.\n"
        f"Cite chunk ids inline like [pX:cY].\n\nContext:\n{context_text}\n\n"
...
def tool_extract_skills(context_chunks: List[RetrievedChunk]) -> Dict[str, Any]:
    ...
    citations_hint = ids_to_brackets(context_chunks)
    prompt = (
        f"List ONLY technical skills present in the text.\n"
        f"Group into: Languages, Data/Big Data, Cloud, Orchestration, DevOps/Other.\n"
        f"Cite chunks like [pX:cY].\n{context_text}\n(Use citations like {citations_hint})"
    )
    out = chat_openai([
...
def tool_compare_with_jd(context_chunks: List[RetrievedChunk], jd_text: str) -> Dict[str, Any]:
    ...
    citations_hint = ids_to_brackets(context_chunks)
    prompt = (
        f"Compare resume vs JD. Output: Fit score (0-100), Matching skills (with citations), "
        f"Gaps, 3 bullets to improve.\nResume:\n{context_text}\n\nJD:\n{jd_text}\n"
        f"(Use citations like {citations_hint})"
    )
    out = chat_openai([
...
def tool_refine_query(question: str, last_answer: str) -> List[str]:
    ...
    user = (
        f"Given the question and last answer, propose 3 refined retrieval queries.\n"
        f'Return as JSON: {{"queries": ["...", "...", "..."]}}\n'
...
def tool_find_calendar_slots(_):
...
If task is complete, set tool="finish" and put final answer in args.final.
Return JSON ONLY.
"""
...
def critic_grounding(answer_text: str, used_chunk_ids: List[str]) -> Dict[str, Any]:
    ...
    user = (
        f"Check grounding of this answer against chunk ids: {used_chunk_ids}.\n"
        f'Return JSON: {{"supported": [...], "unsupported": [...], "confidence": 0.0}}\n'
        f"Answer:\n{answer_text}"
    )
    obj = json_from_model(sys, user)
    ...
    for step in range(policy.max_steps):
        ...
        if tool_name == "finish":
            ...
            if tool_name == "compare_with_jd":
                jd = args.get("jd")
                if not jd:
                    print("\n[Planner chose compare_with_jd] Paste the Job Description:")
                    jd = input().strip()
            ...
            if weak:
                ...
                retrieved = retrieve(collection, refinements[0], n_results=4) or retrieved
                scratchpad.append({"step": step, "tool": "refine_query", "obs": f"retrieved={len(retrieved)}"})
                continue

        elif tool_name == "write_email":
            result = tool_write_email_to_file(
                args.get("subject", "Exploring the Data Engineer role"),
                args.get("body", "Hello,\n\nI'd love to connect regarding the role...\n"),
                args.get("path", "./output_email.txt")
            )
            last_answer = result["output"]
            scratchpad.append({"step": step, "tool": tool_name, "obs": last_answer})
